describe('Navigation', () => {
  ...
  it('should navigate to all main sections', () => {
    ...
    cy.get('body').then($body => {
      ...
      if (bodyText.includes('Personalización') || bodyText.includes('LoginRequired') || bodyText.includes('Iniciar sesión')) {
    ...
    cy.get('body').then($body => {
      const bodyText = $body.text();
      if (bodyText.includes('Mis Informes') || bodyText.includes('No tienes informes') || bodyText.includes('LoginRequired')) {
    ...
    cy.get('body').then($body => {
      const bodyText = $body.text();
      if (bodyText.includes('Deportes') && bodyText.includes('Salud') && bodyText.includes('Grasa')) {
  ...
  it('should work on mobile viewport', () => {
    ...
    cy.get('body').then($body => {
      const bodyText = $body.text();
      if (bodyText.includes('Inicio') && bodyText.includes('Personalizar') && bodyText.includes('Informes')) {
  ...
  it('should handle language switching when logged in', () => {
    ...
    cy.get('body').then($body => {
      const hasLanguageSwitch = $body.find('[data-testid="language-switch"]').length > 0;
      if (hasLanguageSwitch) {
        ...
        cy.log('LanguageSwitch not found, checking if user dropdown is visible');
