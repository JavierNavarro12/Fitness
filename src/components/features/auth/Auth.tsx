...
const Auth: React.FC<AuthProps> = ({ onAuthSuccess }) => {
  ...
  const InputField = ({ label, type, value, onChange, placeholder, icon }: any) => (
    <div className="flex flex-col gap-2">
      <label className="text-gray-800 dark:text-gray-200 font-semibold">{label}</label>
      <div className="flex items-center bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl px-3 focus-within:ring-2 focus-within:ring-red-500 transition-all duration-200 h-12">
        {icon}
        <input
          type={type}
          value={value}
          ...
          className="bg-transparent w-full h-full outline-none ml-2 text-gray-800 dark:text-gray-100 placeholder-gray-400"
          required={label !== 'Nombre' && label !== 'Apellido'}
        />
      </div>
    </div>
  );
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900 p-4">
      <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8 space-y-6">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-red-700 dark:text-red-400">{isLogin ? t('Iniciar Sesión') : t('Crear Cuenta')}</h1>
        </div>
        <form onSubmit={handleAuth} className="space-y-6">
          ...
          <InputField label={t('Correo electrónico')} type="email" value={email} onChange={(e:any) => setEmail(e.target.value)} placeholder="email@example.com" />
          <InputField label={t('Contraseña')} type="password" value={password} onChange={(e:any) => setPassword(e.target.value)} placeholder="••••••••" />
          ...
          {message && <div className="text-green-600 dark:text-green-400 text-center font-medium">{message}</div>}
          {error && <div className="text-red-600 dark:text-red-400 text-center font-medium">{error}</div>}

          <button type="submit" disabled={loading} className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-4 rounded-xl shadow-lg transition-all duration-300 disabled:bg-red-400 disabled:cursor-not-allowed">
        ...
        <div className="flex items-center justify-center">
          ...
          <span className="flex-shrink mx-4 text-gray-500 dark:text-gray-400 text-sm">{t('O continuar con')}</span>
          <div className="flex-grow border-t border-gray-300 dark:border-gray-600"></div>
        </div>

        <button onClick={handleGoogleSignIn} disabled={loading} className="w-full flex items-center justify-center gap-3 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 font-bold py-3 px-4 rounded-xl shadow-sm hover:bg-gray-50 dark:hover:bg-gray-600 transition-all duration-300">
